...
void sock_setbufs(int icmp_sock, int alloc)
{
	...
	if (!sndbuf)
		sndbuf = alloc;
	setsockopt(icmp_sock, SOL_SOCKET, SO_SNDBUFFORCE, (char *)&sndbuf, sizeof(sndbuf));

	rcvbuf = hold = alloc * preload;
	if (hold < 65536)
		hold = 65536;
	setsockopt(icmp_sock, SOL_SOCKET, SO_RCVBUFFORCE, (char *)&hold, sizeof(hold));
	if (getsockopt(icmp_sock, SOL_SOCKET, SO_RCVBUFFORCE, (char *)&hold, &tmplen) == 0) {
		if (hold < rcvbuf)
...
void safe_strcpy(char *s, char* d) {
